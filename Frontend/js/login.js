/**
 * ==========================================================================
 * CONTROLADOR DE AUTENTICACIÓN Y CAPTCHA CORPORATIVO (login.js)
 * ==========================================================================
 */
...
// Función global que es llamada desde el enrutador principal (main.js)
function inicializarLogin() {
    console.log("🔑 Inicializando lógica de login.js con Captcha...");

    const formLogin = document.getElementById("formLogin");
    const feedback = document.getElementById("loginFeedback");
    const btnVolver = document.getElementById("btnVolverInicio");
    
    // Elementos del Captcha agregados en el HTML
    ...
    // Inicializa el primer código al montar la vista
    generarCaptcha();

    // Evento manual para renovar el captcha
    if (btnRefresh) {
        btnRefresh.onclick = () => {
            generarCaptcha();
    ...
    if (btnVolver) {
        btnVolver.onclick = () => {
            navegarA(''); // Regresa a la raíz limpia y muestra la bienvenida
        };
    }
    ...
    if (formLogin) {
        formLogin.addEventListener("submit", (e) => {
            ...
            const captchaInput = document.getElementById("txtCaptchaInput").value.trim().toUpperCase();
            if (captchaInput !== captchaGenerado) {
                mostrarFeedback(feedback, "⚠️ El código de validación Captcha es incorrecto.", "#e74c3c");
            ...
            // Datos de tus inputs originales
            ...
            .then(data => {
                if (data.status === "success") {
                    ...
                    // Limpiar el campo del captcha para futuros logins
                    document.getElementById("txtCaptchaInput").value = "";
                    
                    // Esperamos 1.5s para que el usuario vea el mensaje y navegamos al menú
                    setTimeout(() => {
                        navegarA(data.redirect); 
                    }, 1500);
                } else {
                    mostrarFeedback(feedback, data.message, "#e74c3c");
                    generarCaptcha(); // Renueva el código tras un intento de contraseña fallido
                    document.getElementById("txtCaptchaInput").value = "";
                }
            })
            .catch(error => {
